...
- 资料对外分享：教师将老师资料生成分享链接，通过安全码保护后分享给校外人员或其他班级，无需登录即可访问；支持设置有效期和权限控制（仅预览或允许下载）。
...
- 资料分享：教师可将老师资料空间中的文件或文件夹生成分享链接，支持设置访问权限（仅查看/允许下载）、有效期（1/7/30天、永久、自定义）和安全码保护；访客通过链接和安全码免登录访问，可浏览目录、预览和下载文件；教师可在分享管理界面查看所有分享记录、复制链接、编辑权限和有效期、停用/启用或删除分享。
...
- 系统与账号：教师个人设置、端口配置、上传面板开关、单账号登录控制、老师账号管理、统一日志审计；登录与操作记录按时间线合并展示，账号优先显示对应姓名，可按类型、账号、身份、IP 和时间范围筛选；下载资料、作业附件、提交文件和提交包会在说明中区分，并通过标签辅助快速识别。
...
内置默认密码只适合本机首次运行和演示。正式部署前建议通过环境变量覆盖默认初始密码：

```powershell
$env:CLASSDRIVE_DEFAULT_TEACHER_PASSWORD = "your-strong-password"
$env:CLASSDRIVE_DEFAULT_STUDENT_RESET_PASSWORD = "your-reset-password"
```
...
1. 进入“设置”-“日志审计”，按日志类型、关键词、IP、身份、结果和时间范围筛选。
2. 列表按时间合并显示登录与操作日志，账号列优先显示对应姓名；类型和下载/提交类动作会用标签区分。
3. 使用首页、尾页、页码和跳页输入快速定位历史记录，需要留档时可导出当前筛选结果。
4. 排查同一 IP 切换不同账号的异常情况时，重点对照登录记录，以及“学生下载资料”“学生下载作业附件”“学生下载本人提交文件”“老师下载学生提交文件”“老师下载作业提交包”等操作说明。
